...
Module::Module(std::string moduleName, std::string des, Category c, int k) {
    ...
    registerSetting(new EnumSetting("Toggle", "How module should be toggled", {"Press", "Hold"},
                                    &toggleMode, 0));
}
...
void Module::setEnabled(bool enable) {
    if(enable && !hasPermission()) {
        GI::DisplayClientMessage("%s[Boost]%s Permission denied (need %s)", MCTF::GOLD, MCTF::WHITE,
                                 requiredGroup.c_str());
        return;
    }
...
void Module::onRenderNameTag(Actor* actor, Vec3<float>* pos, bool unknownFlag, float delta,
                             mce::Color* color) {}
...
void Module::onSendPacketWithCancel(Packet* packet, bool* cancel) {}
