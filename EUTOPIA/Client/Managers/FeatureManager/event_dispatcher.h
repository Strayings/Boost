...
enum class standard_event_priority { FIRST, NORMAL, LAST };
}  // namespace detail
...
struct listener_list_concurrent_iterator {
    ...
    listener_list_concurrent_iterator(base_iterator base,
                                      std::shared_lock<std::shared_mutex>&& writeLock)
    ...
    auto operator++(int) const {
        return listener_list_concurrent_iterator{++mBase, this->mWriteLock};
    };

    friend bool operator==(const listener_list_concurrent_iterator& a,
                           const listener_list_concurrent_iterator& b) {
...
struct listener_list {
    ...
    iterator begin() {
        return iterator(mListeners.begin(), std::shared_lock(mWriteLock));
    }
    iterator end() {
        return iterator(mListeners.end(), std::shared_lock(mWriteLock));
    }
...
struct dispatcher {
    ...
    void listen(class_t* instance) {
        ...
        wrapper_t wrapper = [instance](event_t& e) { (instance->*handler)(e); };
        mListeners[priority].emplace_back(instance, std::move(wrapper),
                                          entt::type_hash<decltype(handler)>::value());
    }

    template <auto priority = event_priority_traits<event_priority>::default_value,
              typename wrapper_t = event_wrapper<event_t>>
    void listen(auto handler) {
        std::unique_lock lock(mMutex);
        wrapper_t wrapper = [handler](event_t& e) { handler(e); };
        mListeners[priority].emplace_back(nullptr, std::move(wrapper),
                                          entt::type_hash<decltype(handler)>::value());
    }
...
struct event_dispatcher {
    ...
    template <typename event_t, auto handler,
              auto priority = nes::event_priority_traits<event_priority>::default_value,
